...
export default function PostCard({ post, author }: { post: FeedPost; author?: PostAuthor | null }) {
  ...
  return (
    <article className="bg-white ring-1 ring-emerald-900/5 shadow-sm rounded-2xl p-4">
      {author ? (
        <div className="flex items-center gap-3 mb-2">
          <Link href={`/u/${author.username}`} className="shrink-0">
            {author.headshotUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={author.headshotUrl} alt={displayName} className="w-9 h-9 rounded-full object-cover ring-1 ring-emerald-900/10" />
            ) : (
          ...
          <div className="min-w-0">
            <Link href={`/u/${author.username}`} className="text-sm font-semibold text-slate-900 hover:text-emerald-700">
              {displayName}
            </Link>
        ...
        <p className="text-xs text-slate-400 mb-2">{when}</p>
      )}
      ...
      {post.mediaUrl && post.mediaType === "image" && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={post.mediaUrl} alt="" className="mt-3 w-full rounded-xl object-cover ring-1 ring-emerald-900/5" />
      )}
      ...
      {yt ? (
        ...
        post.linkUrl && (
          <a
            ...
            className="mt-3 block truncate text-sm text-emerald-700 hover:underline"
          >
            🔗 {post.linkUrl}
          </a>
        )
      ...
      {tags.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {tags.map((t) => (
            <span key={t} className="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700">
              #{t}
            </span>
          ))}
        </div>
