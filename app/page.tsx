...
export default async function Home() {
  ...
  if (session?.isLoggedIn) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-[#f0faf5] via-white to-[#eef7f1] dark:from-[#0b1220] dark:via-[#0b1220] dark:to-[#0b1220] text-foreground flex items-center justify-center px-4 py-10">
        <div className="mx-auto w-full max-w-2xl rounded-3xl bg-surface p-8 text-center shadow-xl shadow-emerald-900/5 ring-1 ring-line">
          <p className="text-sm uppercase tracking-[0.35em] text-emerald-600 dark:text-emerald-400 mb-2">Signed in</p>
          <h1 className="text-3xl font-semibold mb-4">@{session.username}</h1>
          <p className="text-muted mb-8">Quick access to your dashboard and cards.</p>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#f0faf5] via-white to-[#eef7f1] dark:from-[#0b1220] dark:via-[#0b1220] dark:to-[#0b1220] text-foreground overflow-hidden">
      {/* Hero */}
      <section className="relative mx-auto max-w-6xl px-4 pt-12 pb-20 grid gap-14 lg:grid-cols-2 items-center">
        ...
        <div className="relative animate-fade-up">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-600 dark:text-emerald-400">Digital networking, reimagined</p>
          <h1 className="mt-4 text-4xl sm:text-5xl font-bold tracking-tight leading-[1.08]">
            Your whole professional self, in <span className="text-emerald-600 dark:text-emerald-400">one tap</span>.
          </h1>
          <p className="mt-5 text-lg text-body max-w-md">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            ...
            <Link href="/p/ava-chen" className="inline-flex items-center justify-center rounded-2xl border border-emerald-200 dark:border-emerald-500/30 bg-surface px-6 py-3.5 text-sm font-semibold text-body transition-all duration-300 hover:border-emerald-300 hover:bg-emerald-50">
              See a live demo →
            </Link>
          </div>
          <p className="mt-5 text-sm text-muted">No download. Works on every phone.</p>
        </div>
        ...
        <div className="relative flex justify-center lg:justify-end animate-fade-up [animation-delay:120ms]">
          <div className="relative">
            ...
            <div className="absolute -bottom-6 -left-6 rounded-2xl bg-surface p-2.5 shadow-xl ring-1 ring-line">
      ...
      <section className="mx-auto max-w-6xl px-4 py-8">
        <div className="grid gap-5 sm:grid-cols-3">
          {[
            ...
            <div
              key={f.title}
              className="animate-fade-up rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-line"
              style={{ animationDelay: `${i * 80}ms` }}
            >
              <div className="text-3xl">{f.emoji}</div>
              <h3 className="mt-3 font-semibold text-foreground">{f.title}</h3>
              <p className="mt-1 text-sm text-body leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-14">
        <div className="text-center mb-10">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-600 dark:text-emerald-400">Try a live card</p>
        ...
        <div className="grid gap-8 sm:grid-cols-3 justify-items-center">
          {samples.map((s, i) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-emerald-700 dark:text-emerald-300 group-hover:text-emerald-600">Open {s.name.split(" ")[0]}&apos;s card →</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-16">
        <div className="rounded-[2rem] bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-14 text-center text-white shadow-xl shadow-emerald-900/10">
          ...
          <Link href="/signup" className="mt-7 inline-flex items-center justify-center rounded-2xl bg-surface px-7 py-3.5 text-sm font-semibold text-emerald-700 dark:text-emerald-300 transition-all duration-300 hover:bg-emerald-50 active:scale-[0.98]">
            Create your free card
          </Link>
        </div>
      </section>
...
function MiniCard({
  ...
  return (
    <div className="w-64 rounded-3xl bg-surface shadow-xl shadow-emerald-900/10 ring-1 ring-line overflow-hidden">
      ...
      <div className="px-5 pb-5 -mt-8 flex flex-col items-center text-center">
        ...
        <img src={headshot} alt={name} className="w-16 h-16 rounded-full object-cover ring-4 ring-white shadow bg-surface" />
        <p className="mt-2 font-bold text-foreground">{name}</p>
        <p className="text-xs text-muted">{title}{company ? ` · ${company}` : ""}</p>
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5 w-full text-xs">
          <div className="rounded-lg border border-emerald-200 dark:border-emerald-500/30 py-1.5">📞</div>
          <div className="rounded-lg border border-emerald-200 dark:border-emerald-500/30 py-1.5">✉️</div>
          <div className="rounded-lg border border-emerald-200 dark:border-emerald-500/30 py-1.5">🔗</div>
        </div>
      </div>
    </div>
